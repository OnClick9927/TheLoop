...
#pragma warning disable CS1591 // 缺少对公共可见类型或成员的 XML 注释
    public class Packet
    {
        ...
        public bool UnPack(byte[] buffer, int offset, int size)
        {
            ...
            if (dst.Length < HelpBuffLen - 2) return false;
            ...
            if (plen > dst.Length - HelpBuffLen + 2) return false;
            if (_Head == null) _Head = new PacketHeader();
            _Head.MainId = dst.ToUInt32(0);
            _Head.SubId = dst.ToUInt32(4);
            _Head.pkgType = dst[8];
            _Head.pkgCount = dst.ToUInt16(9);
            _Head.messageLen = plen;
