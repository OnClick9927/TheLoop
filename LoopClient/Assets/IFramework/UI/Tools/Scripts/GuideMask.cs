...
namespace IFramework.UI
{
    ...
    public class GuideMask : Graphic, IPointerClickHandler
    {
        ...
        void IPointerClickHandler.OnPointerClick(PointerEventData eventData)
        {
            if (raycastPass)
            {
                if (raycastInRect)
                {
                    if (Contains(rect, eventData.position))
                    {
                        DoUIEvent(eventData);
                    }
                }
        ...
        {
            (this as IPointerClickHandler).OnPointerClick(new PointerEventData(EventSystem.current) { position = position });
        }
        private void OnClick()
        {
            onClick.Invoke();
        }
        private void DoUIEvent(PointerEventData eventData)
        {
            ...
            {
                if (current == results[i].gameObject) continue;
