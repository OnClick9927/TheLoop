...
namespace IFramework.Hotfix.Asset
{
    partial class AssetsWindow
    {
        ...
        {
            private class Directorys : TreeView
            {
                public Directorys(TreeViewState state) : base(state)
                {
                    this.Reload();
                ...
                protected override IList<TreeViewItem> BuildRows(TreeViewItem root)
                {
                    ...
                    var paths = buildSetting.GetBuildPaths();
                ...
                protected override void RowGUI(RowGUIArgs args)
                {
                    ...
                    {
                        buildSetting.RemoveBuildPath(args.item.displayName);
                        Reload();
                    }
                }
                    ...
                    if (info.enterArera && info.dragging)
                    {
                        if (info.paths != null)
                        {
                            for (int i = 0; i < info.paths.Length; i++)
                            {
                                ...
                                if (path.IsDirectory())
                                    buildSetting.AddBuildPath(path);
                            }
                            Reload();
                        }
                    }
